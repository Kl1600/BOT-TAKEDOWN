...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption(o => o.setName('duree').setDescription('Duree optionnelle du ban (ex: 7j, 24h)').setRequired(false).setMaxLength(20))
...
Exemples : \`/ban @user 7j Spam\` ou \`/ban @user Spam\``;
  ...
  try {
    ...
    if (durationMs) {
      ...
      return replyOk(interaction, `? \`${target.user.username}\` a ete banni temporairement.
-# Fin du ban : <t:${unbanAt}:R>
-# Raison : ${raison}`, 0xED4245);
    }
    return replyOk(interaction, `? \`${target.user.username}\` a ete banni.
-# Raison : ${raison}`, 0xED4245);
  } catch (e) {
...
Exemples : \`${message.client.prefix || '+'}ban 123456789012345678 7j Spam\` ou \`${message.client.prefix || '+'}ban 123456789012345678 Spam\``;
  ...
  try {
    ...
    if (durationMs) {
      ...
      return message.reply(`? \`${mention.user.username}\` a ete banni temporairement. Fin du ban : <t:${unbanAt}:R>`);
    }
    await message.reply(`? \`${mention.user.username}\` a ete banni. Raison : ${raison}`);
  } catch (e) {
