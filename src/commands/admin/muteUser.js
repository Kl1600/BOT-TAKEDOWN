...
import { executeMute, hasTicketManagementAccess, replyErr, replyOk, MUTE_DURATIONS } from '../../services/moderationService.js';
...
export async function executeUserContextMenu(interaction) {
  ...
  if (targetId === interaction.user.id) {
    return replyErr(interaction, 'Vous ne pouvez pas vous mettre en sourdine vous-même.');
  }
...
export async function handleMuteUserModalSubmit(interaction) {
  ...
  const seconds = MUTE_DURATIONS[dureeRaw];
  const raison = interaction.fields.getTextInputValue('raison')?.trim() || 'Aucune raison fournie';

  if (!seconds) {
    return replyErr(interaction, 'Durée invalide. Exemples : 60s, 5min, 1h, 7j.');
  }
  ...
  try {
    ...
    return replyOk(
      interaction,
      `✅ \`${target.user.username}\` est en sourdine pendant **${dureeRaw}**.\n-# Expire : <t:${Math.floor(until.getTime() / 1000)}:R>\n-# Raison : ${raison}`,
      0xF0A500
    );
  } catch (error) {
    return replyErr(interaction, error?.message || 'Impossible de mettre ce membre en sourdine.');
  }
}
