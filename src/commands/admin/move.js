...
export default {
  data: new SlashCommandBuilder()
    ...
    option
        .setName('id')
        .setDescription('ID Discord du membre à retirer')
        .setRequired(true)
    ),
  async executeSlash(interaction) {
    ...
    if (!removed) {
      return replyErr(interaction, "Ce membre n'a pas accès à ce ticket.");
    }
  ...
  async executePrefix(message, args) {
    ...
    if (!removed) {
      return prefixReply(message, "❌ Ce membre n'a pas accès à ce ticket.");
    }
